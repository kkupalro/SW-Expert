...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		for(int i = 0; i < N; i++)
		{
			...
			int x = Integer.parseInt(st.nextToken()); // x ÁÂÇ¥ (0 ~ 100)
			int y = Integer.parseInt(st.nextToken()); // y ÁÂÇ¥ (0 ~ 100)
			int d = Integer.parseInt(st.nextToken()); // d ¹æÇâ (> ^ < v) (0 ~ 3)
			int g = Integer.parseInt(st.nextToken()); // ¼¼´ë (0 ~ 10)
			solve(x, y, d, g);
		}
		for(int i = 0; i < 100; i++)
		{
